...
#define NGX_RTMP_DASH_BUFSIZE           (1024*1024*5)
...
ngx_rtmp_dash_close_fragment(ngx_rtmp_session_t *s, ngx_rtmp_dash_track_t *t)
{
    ...
    *ngx_sprintf(ctx->stream.data + ctx->stream.len, "%uD.m4%c",
    ...
    while (left > 0) {
        ...
        if (n == NGX_ERROR || n == 0) {
            ngx_log_error(NGX_LOG_ERR, s->connection->log, ngx_errno, 
                       "dash: error read file because is 0,Gleb was right"); 
            break;
        }
    ...
    if (fd != NGX_INVALID_FILE) {
        if(ngx_close_file(fd) == NGX_FILE_ERROR) {
           ngx_log_error(NGX_LOG_ERR, s->connection->log, ngx_errno,
                        ngx_close_file_n "dash: file close failed 1");
        }
    }

    if(ngx_close_file(t->fd) == NGX_FILE_ERROR) {
       ngx_log_error(NGX_LOG_ERR, s->connection->log, ngx_errno,
                    ngx_close_file_n "dash: file close failed 2");
    }
