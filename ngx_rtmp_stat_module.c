...
#define NGX_RTMP_STAT_SECURE        0x10
/*
...
static ngx_conf_bitmask_t           ngx_rtmp_stat_masks[] = {
    ...
    { ngx_string("secure"),         NGX_RTMP_STAT_SECURE        },
    { ngx_null_string,              0 }
};
...
static ngx_command_t  ngx_rtmp_stat_commands[] = {
    ...
    { ngx_string("rtmp_stat"),
        ...
        ngx_rtmp_stat_masks },
    { ngx_string("rtmp_stat_secure"),
        NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE,
        ngx_rtmp_stat,
        NGX_HTTP_LOC_CONF_OFFSET,
        offsetof(ngx_rtmp_stat_loc_conf_t, stat_secure),
        ngx_rtmp_stat_masks },
...
ngx_rtmp_stat_client(ngx_http_request_t *r, ngx_chain_t ***lll,
    ngx_rtmp_session_t *s, int secure)
{
    u_char  buf[NGX_INT_T_LEN];
    ...
    NGX_RTMP_STAT_L("<address>");
    NGX_RTMP_STAT_L("HIDDEN FOR SECURITY REASONS");
    NGX_RTMP_STAT_L("</address>");   
    }
    ...
    for (n = 0; n < lacf->nbuckets; ++n) {
        for (stream = lacf->streams[n]; stream; stream = stream->next) {
            ...
            for (ctx = stream->ctx; ctx; ctx = ctx->next, ++nclients) {
                s = ctx->session;
                if (slcf->stat & NGX_RTMP_STAT_CLIENTS || slcf->stat_secure) {
                    NGX_RTMP_STAT_L("<client>");

                  if (slcf->stat_secure){
                    ngx_rtmp_stat_client(r, lll, s, 1);
                   }
                   else{
                       ngx_rtmp_stat_client(r, lll, s, 0);
                   }
        ...
        ngx_rtmp_core_app_conf_t *cacf)
{
    ...
    if (slcf->stat & NGX_RTMP_STAT_LIVE || slcf->stat_secure) {
        ngx_rtmp_stat_live(r, lll,
                cacf->app_conf[ngx_rtmp_live_module.ctx_index]);
    }

    if (slcf->stat & NGX_RTMP_STAT_PLAY || slcf->stat_secure) {
...
ngx_rtmp_stat_handler(ngx_http_request_t *r)
{
    ...
    slcf = ngx_http_get_module_loc_conf(r, ngx_rtmp_stat_module);
    if (slcf->stat == 0 && slcf->stat_secure == 0) {
        return NGX_DECLINED;
    }
